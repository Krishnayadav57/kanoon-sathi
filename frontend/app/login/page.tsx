    ...
    <div className="flex min-h-[85vh] items-center justify-center px-4 py-12">
      <div className="w-full max-w-md animate-fade-up">
        <div className="mb-8 flex flex-col items-center text-center">
          ...
          <h1 className="mt-5 font-display text-2xl font-semibold text-ink">{t("login_title")}</h1>
        ...
        <div className="rounded-3xl border border-slate-100 bg-white p-8 shadow-soft">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <Input label={t("email")} id="email" type="email" required autoComplete="email" value={email} onChange={(e) => setEmail(e.target.value)} />
            <div className="relative">
              <Input label={t("password")} id="password" type={showPw ? "text" : "password"} required autoComplete="current-password" value={password} onChange={(e) => setPassword(e.target.value)} className="pr-10" />
          ...
          <p className="mt-5 text-center text-sm text-slate-500">
            {t("no_account")}{" "}
            <Link href="/register" className="font-semibold text-crimson-500 hover:underline">{t("nav_register")}</Link>
          </p>
        </div>
      </div>
    </div>
